const {
  ...
  router.get('/submissions', async (req, res) => {
    ...
    res.render('submissions/all-submissions', {
      submissions
    });
  });
  ...
  router.get('/submissions/add', async (req, res) => {
    ...
    res.render('submissions/new-submission', {
      courses
    });
  });
  ...
  router.get('/submissions/view/:id', async (req, res) => {
    const submission = await Submission.findById(req.params.id);
    res.render('submissions/submission-detail', {
      submission
    });
  });
  ...
  router.post('/submissions/new-submission', async (req, res) => {
    ...
    if (errors.length > 0) {
      ...
      const submission_id = await submission.findOne({
